...
import { Timer } from './robot.sensors';
...
export interface IDrawable {
    ...
    draw(rCtx: CanvasRenderingContext2D, myRobot: BaseRobot): void;
}
...
export abstract class BaseRobot implements IRobot, ISelectable {
    debug: boolean = false;
    timer: Timer;
    ...
    draw(rCtx: CanvasRenderingContext2D, dt) {
        ...
        let baseMobileRobot: BaseMobileRobot = this as unknown as BaseMobileRobot;
        rCtx.save();
        if (this.mobile) {
        ...
        if (this.selected && this.mobile) {
            let objectCorners = [
                {
                    x: Math.round((this as unknown as BaseMobileRobot).chassis.frontRight.x),
                    y: Math.round((this as unknown as BaseMobileRobot).chassis.frontRight.y),
                },
                {
                    x: Math.round((this as unknown as BaseMobileRobot).chassis.backRight.x),
                    y: Math.round((this as unknown as BaseMobileRobot).chassis.backRight.y),
                },
                {
                    x: Math.round((this as unknown as BaseMobileRobot).chassis.backLeft.x),
                    y: Math.round((this as unknown as BaseMobileRobot).chassis.backLeft.y),
                },
                {
                    x: Math.round((this as unknown as BaseMobileRobot).chassis.frontLeft.x),
                    y: Math.round((this as unknown as BaseMobileRobot).chassis.frontLeft.y),
                },
            ];
    ...
    updateActions(myRobot: BaseRobot, dt: number, interpreterRunning: boolean): void {
    ...
    updateSensors(running: boolean, dt: number, uCtx: CanvasRenderingContext2D, udCtx: CanvasRenderingContext2D, personalObstacleList: any[]): void {
        let values = this.interpreter.getRobotBehaviour().hardwareState.sensors;
        for (const item in this) {
            if (this[item] && (this[item] as unknown as IUpdateSensor).updateSensor) {
                (this[item] as unknown as IUpdateSensor).updateSensor(running, dt, this, values, uCtx, udCtx, personalObstacleList);
            }
        }
    }
    ...
    protected extendMouseEvent(e): void {
        let X = e.clientX || e.originalEvent.touches[0].pageX;
        let Y = e.clientY || e.originalEvent.touches[0].pageY;
        let top = $('#robotLayer').offset().top;
        let left = $('#robotLayer').offset().left;
        let scale = Simulation.Instance.scale;
    ...
    private drawLabel(dt: number) {
        ...
        if (this.mobile) {
            $('#robotIndex').css('background-color', (this as unknown as BaseMobileRobot).chassis.geom.color);
        }
        if (this.name !== undefined) {
            $systemValuesView.append('<div><label>Program Name</label><span>' + this.name + '</span></div>');
        }
        ...
        if (this.mobile) {
            ...
            $systemValuesView.append('<div><label>Robot X</label><span>' + UTIL.round((this as unknown as BaseMobileRobot).pose.x / 3, 1) + '</span></div>');
            $systemValuesView.append('<div><label>Robot Y</label><span>' + UTIL.round((this as unknown as BaseMobileRobot).pose.y / 3, 1) + '</span></div>');
            $systemValuesView.append(
                '<div><label>Robot θ</label><span>' + UTIL.round((this as unknown as BaseMobileRobot).pose.getThetaInDegree(), 0) + '°</span></div>'
            );
        }
...
export class RobotFactory {
    ...
    static async createRobots(
        ...
        robotType: string
    ): Promise<{ robots: BaseMobileRobot[]; robotClass: any }> {
        $('#simRobotContent').html('');
        let myRobotClass = await import('EV3Robot');
        let myRobots: BaseMobileRobot[] = [];
        $('#constantValue').html('');
        interpreters.some((interpreter, index) => {
            if (index > C.MAX_SIM_ROBOTS - 1) {
                alert(
