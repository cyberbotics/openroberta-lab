import { IDestroyable, ISelectable, SelectionListener } from './BaseRobot';
...
export abstract class BaseSimulationObject implements ISelectable, ISimulationObstacle, IDestroyable {
    ...
    protected constructor(myId: number, myScene: Scene, mySelectionListener: SelectionListener, type: SimObjectType, optColor?: string | typeof Image) {
        this.myId = myId;
...
export class RectangleSimulationObject extends BaseSimulationObject {
    ...
    handleMouseMove(e: JQuery.TouchEventBase): void {
        ...
        if (mySelectedCornerIndex >= 0 && this.selected) {
            if (this.w >= C.MIN_SIZE_OBJECT && this.h >= C.MIN_SIZE_OBJECT) {
                switch (mySelectedCornerIndex) {
                    case 0:
                        this.x += dx;
                        this.y += dy;
            ...
            } else if (this.w < C.MIN_SIZE_OBJECT) {
                if (mySelectedCornerIndex == 0 || mySelectedCornerIndex == 3) {
                    this.x -= C.MIN_SIZE_OBJECT - this.w;
                }
                this.w = C.MIN_SIZE_OBJECT;
            } else if (this.h < C.MIN_SIZE_OBJECT) {
                if (mySelectedCornerIndex == 1 || mySelectedCornerIndex == 2) {
                    this.y -= C.MIN_SIZE_OBJECT - this.h;
                }
                this.h = C.MIN_SIZE_OBJECT;
            }
            this.updateCorners();
        } else {
            if (this.isDown) {
...
export class CircleSimulationObject extends BaseSimulationObject {
    ...
    handleMouseMove(e: JQuery.TouchEventBase): void {
        ...
        if (mySelectedCornerIndex >= 0 && this.selected) {
            ...
            this.r = Math.max(this.r, C.MIN_SIZE_OBJECT);
            this.updateCorners();
        } else {
            if (this.isDown) {
                this.x += dx;
