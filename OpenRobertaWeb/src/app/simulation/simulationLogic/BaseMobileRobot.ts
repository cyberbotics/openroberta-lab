import { BaseRobot, ISelectable, SelectionListener } from './BaseRobot';
...
import { GyroSensor } from './robot.sensors';
...
export abstract class BaseMobileRobot extends BaseRobot {
    abstract chassis: ChassisDiffDrive;
    /*protected colorSensors: ColorSensor[] = [];
    protected gyroSensors: GyroSensor[] = [];
    protected infraSensors: DistanceSensor[] = [];
    protected lightSensors: ColorSensor[] = [];
    protected touchSensors: TouchSensor[] = [];
    protected ultraSensors: DistanceSensor[] = [];*/
        ...
        if (this.selected) {
            ...
            switch (keyName) {
                ...
                case 'ArrowLeft':
                    ...
                    Object.keys(this)
                        .filter((x: any) => x instanceof GyroSensor)
                        .forEach((gyro) => ((gyro as unknown as GyroSensor).angleValue -= SIMATH.toDegree(angleLeft)));
                    e.preventDefault();
                    e.stopPropagation();
                    break;
                case 'ArrowDown':
                ...
                case 'ArrowRight':
                    ...
                    Object.keys(this)
                        .filter((x: any) => x instanceof GyroSensor)
                        .forEach((gyro) => ((gyro as unknown as GyroSensor).angleValue += SIMATH.toDegree(angleRight)));
                    e.preventDefault();
                    e.stopPropagation();
                    break;
                default:
