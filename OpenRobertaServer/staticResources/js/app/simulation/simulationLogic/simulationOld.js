 ...
 */
define(["require", "exports", "simulation.scene", "simulation.constants", "util", "interpreter.interpreter", "interpreter.robotSimBehaviour", "volume-meter", "message", "jquery", "huebee", "blockly", "nn.controller", "./BaseMobileRobot", "./BaseRobot", "./simulation.objects"], function (require, exports, simulation_scene_1, simulation_constants_1, UTIL, SIM_I, ROBOT_B, Volume, MSG, $, HUEBEE, Blockly, NN_CTRL, BaseMobileRobot_1, BaseRobot_1, simulation_objects_1) {
    ...
    function loadBackgroundImages(onFirstLoaded) {
                ...
                if (UTIL.isLocalStorageAvailable()) {
                    ...
                    if (customBackground) {
                        ...
                        if (currentTimestamp - customBackground.timestamp > 63 * 24 * 60 * 60 * 1000) {
                            ...
                            var customBackgroundIndex = imgList.length;
        ...
        while (i < imgList.length) {
            var img = (imgBackgroundList[i] = new Image());
            img.onload = onload;
            img.onerror = function (e) {
                console.error(e);
            };
    ...
    function addObstacle(shape) {
        var obstacle = addObject(shape, simulation_objects_1.SimObjectType.Obstacle, obstacleList);
        /*if (selectedColorArea >= 0) {
            highLightCorners = [];
            updateColorAreaLayer();
        }
    ...
    function addColorArea(shape) {
        var colorArea = addObject(shape, simulation_objects_1.SimObjectType.ColorArea, colorAreaList);
        /*if (selectedObstacle >= 0) {
            highLightCorners = [];
            updateObstacleLayer();
        }
    ...
    function addObject(shape, type, objectList) {
        ...
        var newObject = simulation_objects_1.SimObjectFactory.getSimObject(myListener, shape, type, { x: x, y: y });
        objectList.push(newObject);
        enableChangeObjectButtons();
        return newObject;
    }
    function changeColorWithColorPicker(color) {
        var objectList = obstacleList.concat(colorAreaList); // >= 0 ? obstacleList[selectedObstacle] : selectedColorArea >= 0 ? colorAreaList[selectedColorArea] : null;
        var myObj = objectList.filter(function (obj) {
            return obj.selected;
        });
        myObj[0].color = color;
    }
    ...
    function init(programs, refresh, robotType) {
        myListener = (myListener && myListener.clean()) || new BaseRobot_1.SelectionListener();
        mouseOnRobotIndex = -1;
        ...
        if (refresh) {
            ...
            robots = [];
            obstacleList = [];
            obstacleList.push(simulation_objects_1.SimObjectFactory.getSimObject(myListener, simulation_objects_1.SimObjectShape.Circle, simulation_objects_1.SimObjectType.Obstacle, { x: 100, y: 100 }));
                ...
                if (robots[0].idle) {
                    ...
                    images = loadImages(['obstacle1', 'pattern', 'ruler'], ['baustelle' + imgType, 'wallPattern.png', 'ruler' + imgType], function () {
                        loadBackgroundImages(function () {
                            ...
                            setRuler();
                            ground = new simulation_objects_1.Ground(10, 10, imgBackgroundList[currentBackground].width, imgBackgroundList[currentBackground].height);
                            initScene();
                            $('.pace').fadeOut('fast');
                        });
                    });
    ...
    function initScene() {
        cancelAnimationFrame(globalID);
        scene = new simulation_scene_1.default(imgBackgroundList[currentBackground], robots, images.pattern, ruler);
        //scene.drawRobots();
        resetSelection();
        addMouseEvents();
        disableChangeObjectButtons();
    ...
    function createRobot(reqRobot, configuration, num, optYOffset, robotBehaviour) {
        ...
        var robot = new reqRobot(num, configuration, robotBehaviour, myListener);
        if (robot.mobile) {
            switch (currentBackground) {
                case 0: {
                    robot.pose = new BaseMobileRobot_1.Pose(200, 200 + yOffset, 0);
                    //robot.canDraw = false;
                    break;
                }
                case 1: {
                    robot.pose = new BaseMobileRobot_1.Pose({
                        x: 200,
                        y: 200 + yOffset,
                        theta: 0,
                    });
